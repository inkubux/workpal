...
const getReleaseId = async (version) => {
  const {
    ...
    method: "GET",
    url: `https://api.github.com/repos/inkubux/workpal/releases/tags/v${version}`,
    headers: {
...
const uploadArtifact = async () => {
  ...
  console.log(
    `Uploading ${artifactFileName} with version ${version} to release ${releaseId}`
  );
  ...
  const {
    ...
    method: "POST",
    url: `https://uploads.github.com/repos/inkubux/workpal/releases/${releaseId}/assets?name=${artifactFileName}`,
    headers: {
  ...
  if (!assetId) {
    console.error(
      `Error uploading artifact ${artifactFileName} to release ${releaseId}`
    );
    process.exit(1);
  }
  console.log(
    `Artifact ${artifactFileName} with version ${version} to release ${releaseId} successfully uploaded as asset ${assetId}`
  );
};
