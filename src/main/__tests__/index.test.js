...
describe("Main module test suite", () => {
  ...
  beforeEach(() => {
    ...
    userAgentModule.userAgentForView.mockImplementation(
      () => "UserAgent String"
    );
    userAgentModule.initBrowserVersions.mockImplementation(() => ({
      then: (func) => {
        func.call();
        return { catch: () => {} };
      },
  ...
  describe("init - environment preparation", () => {
    ...
    test("fixUserDataLocation, should set a location in lower-case (Electron <14 compatible)", () => {
      ...
      mockApp.getPath.mockImplementation(
        () => "ImMixed-Case/WithSome\\Separator$"
      );
      // When
      main.init();
  ...
  describe("mainWindow events", () => {
    beforeEach(() => {
      ...
      mockBrowserWindow.getContentBounds = jest.fn(() => ({
        x: 0,
        y: 0,
        width: 10,
        height: 34,
    ...
    test("maximize, should set browserview to fit window and store new size in configuration file", () => {
      ...
      expect(singleView.setBounds).toHaveBeenCalledWith({
        x: 0,
        y: 0,
        width: 10,
        height: 34,
    ...
    describe("resize", () => {
      ...
      test("multiple views, should set last browserview to fit window and store new size in configuration file", () => {
        ...
        expect(topBar.setBounds).toHaveBeenCalledWith({
          x: 0,
          y: 0,
          width: 10,
          height: 1,
  ...
  describe("initTabListener ipc events", () => {
    describe("tabsReady", () => {
      ...
      test("Previous saved tabs in loaded settings, should add tabs to manager and activate them as they are added", () => {
        ...
        settingsModule.loadSettings.mockImplementation(() => ({
          tabs: [
            { id: "1337", otherInfo: "A Tab" },
            {
              id: "disabled-1337",
              disabled: true,
              otherInfo: "I should be ignored",
            },
          ],
        }));
        main.init();
        ...
        expect(addTabsNested).toHaveBeenCalledWith([
          { id: "1337", otherInfo: "A Tab", active: false },
        ]);
    ...
    describe("activateTab", () => {
      ...
      beforeEach(() => {
        ...
        tabManagerModule.getTab = jest.fn((id) =>
          id === "validId" ? activeTab : null
        );
      });
      ...
      test("active tab, should resize tab and set it as the main window browser view", () => {
        ...
        // Then
        expect(activeTab.setBounds).toHaveBeenCalledWith({
          x: 0,
          y: 46,
          width: 13,
          height: 37,
        });
        expect(mockBrowserWindow.setBrowserView).toHaveBeenCalledWith(
          mockTabContainer
        );
        expect(mockBrowserWindow.addBrowserView).toHaveBeenCalledWith(
          activeTab
        );
      ...
      test("#23, setBounds should be called AFTER adding view to BrowserWindow", () => {
        ...
        mockIpc.listeners.activateTab({}, { id: "validId" });
        // Then
        expect(mockBrowserWindow.setBrowserView).toHaveBeenCalledBefore(
          mockTabContainer.setBounds
        );
        expect(mockBrowserWindow.addBrowserView).toHaveBeenCalledBefore(
          mockTabContainer.setBounds
        );
        expect(mockBrowserWindow.setBrowserView).toHaveBeenCalledBefore(
          activeTab.setBounds
    ...
    test("notificationClick, should restore window and activate tab", () => {
      ...
      // Then
      expect(mockTabContainer.webContents.send).toHaveBeenCalledWith(
  ...
  describe("initDialogListeners ipc events", () => {
    ...
    test("saveSettings, should reload settings and reset all views", () => {
      ...
      mockIpc.listeners.settingsSave(
        {},
        { tabs: [{ id: 1337 }], enabledDictionaries: [] }
      );
      // Then
      ...
      expect(mockBrowserWindow.removeBrowserView).toHaveBeenCalledWith(
        settingsView
      );
