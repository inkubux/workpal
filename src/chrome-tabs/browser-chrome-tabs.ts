...
const getTabContainer = () =>
  <HTMLDivElement>document.querySelector(".tab-container");
...
const sendActivateTab = (id) =>
  ipcRenderer.send(APP_EVENTS.activateTab, { id });
...
const dispatchActivateTab = (dispatch) => (id) =>
  dispatch({ type: ACTIONS.ACTIVATE_TAB, payload: id });
...
const dispatchSetTabTitle =
  (dispatch) =>
  (_event, { id, title }) => {
    dispatch({
      type: ACTIONS.SET_TAB_PROPERTY,
      payload: {
        id,
        property: "title",
        value: title,
...
const calculateTabWidth = (numberOfTabs: number) => {
  const availableWidth =
    (getChromeTabs() || getTabContainer())?.clientWidth || window.innerWidth;
  const tabsCumulativeOverlappedWidth =
    (numberOfTabs - 1) * TAB_CONTENT_OVERLAP_DISTANCE;
  const targetWidth =
    (availableWidth - 2 * TAB_CONTENT_MARGIN + tabsCumulativeOverlappedWidth) /
    numberOfTabs;
...
const tabStyle = (width: number, idx: number, offsetX: number) =>
...
const Favicon = ({ favicon = "" }) => {
  const faviconProps: { hidden?: boolean; style?: string } = { hidden: true };
  if (favicon) {
...
const NotificationIcon = ({ disableNotifications = false }) =>
...
const Tab = ({
  ...
  const tabDrag = (event) => {
    const newIdx = calculateNewIndex(
      numberOfTabs,
      event.clientX,
      originX,
      width,
  ...
  return html`
    <div class="chrome-tab" ...${props} ...${active && { active: "" }}>
      ...
      <div class="chrome-tab-content">
        <${Favicon} ...${rest} />
        <div class="chrome-tab-title" title=${url}>${title ? title : url}</div>
  ...
  return html`
    ...
    >
      <div class="chrome-tabs-content">
        ${tabs.map(
          (tab, idx) => html` <${Tab}
            ...${{ ...tab, dispatch, numberOfTabs: tabs.length, idx, width }}
          />`
        )}
      </div>
    </div>
...
const Settings = () => html`
  <div class="settings">
    <button class="settings__button button" onclick=${openSettings}>
      <span class="icon">
...
const TabContainer = () => {
  ...
  useLayoutEffect(() => {
    ...
    ipcRenderer.on(APP_EVENTS.activateTabInContainer, (_event, { tabId }) =>
      dispatchActivateTab(dispatch)(tabId)
    );
